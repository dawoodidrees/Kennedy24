...
const alchemyProvider = process.env.ALCHEMY_PROVIDER_URL;
const publicSignerPrivateKey = process.env.SIGNER_PRIVATE_KEY;
const abi = [
  {
    inputs: [],
...
export async function POST(request: NextRequest) {
  ...
  if (collection) {
    const web3Instance = new Web3(alchemyProvider);
...
// const signMessage1 = (nonce: number) => {
//   const msg = ethers.AbiCoder.defaultAbiCoder().encode(
//     ["address", "address", "uint256"],
//     [contractAddress, receiver, nonce]
//   );

//   const signatureSigner = new ethers.Wallet(publicSignerPrivateKey);
//   const signingKey = signatureSigner.signingKey;

//   const msgHash = keccak256(msg);
//   const { r, s, v } = signingKey.sign(msgHash);
...
const signMessage2 = async (
  ...
  var { r, s, v } = await web3Instance.eth.accounts.sign(
    message,
    publicSignerPrivateKey
  );

  return { r, s, v: parseInt(v, 16) };
};
